// copy of record for http://glslsandbox.com/e#44459.0
...
void main( void ) {
	...
	for (float k = 0.; k < n; ++k) { // for each forcing circle
		...
		if (d > 0.3 && d < 0.302) {
			...
			gl_FragColor = live * p + texture2D(backbuffer, position) * (1.-p); // blend with transparency
			done = true;
		}
	}
